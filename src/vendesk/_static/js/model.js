...
var model =
{
  ...
  /**
   * Recarga la página enviando los valores actuales como query string para que los filtros se preserven.
   */
  refresh_leads: function() {
    const url = window.location.href.split('?')[0];
    const qry = new URLSearchParams(this.getDataFilters()).toString();
  ...
  restore_filters_from_URL() {
    ...
    const queryfields_as_filterfields = {
      agent_filter: "cbAgentes",
      status_filter: "cbStatus",
      color_filter: "cbColors",
      pipeline: "cbPipeline",
      stage: "cbStages",
      ordeby: "sOrderBy",
      search: "txtFilter"
    ...
    Object.entries(queryfields_as_filterfields).forEach(entry => {
      const [query_id, field_id] = entry;
      
      let filter = module_filters.querySelector("#"+field_id);
      let value = params[query_id];

      if (filter && value) {
        filter.value = value;

        if (field_id == queryfields_as_filterfields.status_filter) this.div_pipeline.classList.toggle('d-none', Number(value) != 3);
        if (field_id == queryfields_as_filterfields.pipeline && value != "all") this.load_stages(value);
      }
    });
  },
